...
func setUpTime(t *time.Time, formats []string, defValue time.Time) *FlagSet {
	f := NewFlagSet("test", ContinueOnError)
	f.TimeVar(t, "time", defValue, formats, "Time")
	return f
}
...
func TestTime(t *testing.T) {
	...
	for i := range testCases {
		...
		f := setUpTime(&timeVar, formats, time.Time{})
...
func TestTimeDefaultZeroProviderZeroTime(t *testing.T) {
	var timeVar time.Time
	formats := []string{time.RFC3339Nano, time.RFC1123Z}
	fs := setUpTime(&timeVar, formats, time.Time{})
	s := fs.FlagUsages()
	assert.NotContains(t, s, "default")
}

func TestTimeDefaultZeroProviderNonZeroTime(t *testing.T) {
	var timeVar time.Time
	formats := []string{time.RFC3339Nano, time.RFC1123Z}
	fs := setUpTime(&timeVar, formats, time.Now())
	s := fs.FlagUsages()
